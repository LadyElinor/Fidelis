...
def derive_meaning_case(description: str, context: dict[str, Any]) -> TranslationResult:
    ...
    if any(token in text for token in ("bias", "biased", "disparate impact", "women's college", "screening model", "hiring model", "fairness")):
        ...
        return _result(
            ...
            matched_signals=["bias", "disparate impact", "fairness"],
            notes=notes,
        )

    if any(token in text for token in ("opaque model", "black box", "unverifiable", "transparency", "sepsis", "model effectiveness", "proprietary model")):
        ...
        return _result(
            ...
            matched_signals=["black box", "unverifiable", "transparency"],
            notes=notes,
        )

    if any(token in text for token in ("sales pressure", "cross-selling", "unauthorized accounts", "gaming", "metric corruption", "incentive gaming", "blackouts", "market manipulation")):
        ...
        return _result(
            ...
            matched_signals=["gaming", "sales pressure", "unauthorized accounts"],
            notes=notes,
        )

    if any(token in text for token in ("whistleblow", "whistleblower", "classified leak", "public interest disclosure", "snowden", "surveillance leak")):
        ...
        return _result(
            ...
            matched_signals=["whistleblower", "public interest disclosure", "classified leak"],
            notes=notes,
        )

    if any(token in text for token in ("conceal losses", "hidden losses", "barings", "nick leeson", "recovery bet", "unauthorized bets")):
        ...
        return _result(
            ...
            matched_signals=["hidden losses", "recovery bet", "unauthorized bets"],
            notes=notes,
        )

    if any(token in text for token in ("chatbot", "trolls", "manipulated into", "abuse-hardening", "adaptive system", "adversarial public")):
        ...
        return _result(
            ...
            matched_signals=["chatbot", "trolls", "adversarial public"],
            notes=notes,
        )

    if any(token in text for token in ("retraction", "retract paper", "correction of the record", "replication failure", "defamation suit over retraction")):
        ...
        return _result(
            ...
            matched_signals=["retraction", "replication failure", "correction of the record"],
            notes=notes,
        )

    if any(token in text for token in ("self-driving", "autonomous vehicle fatality", "who's at the wheel", "distributed accountability", "safety driver", "disabled automatic braking")):
        ...
        return _result(
            ...
            matched_signals=["self-driving", "distributed accountability", "safety driver"],
            notes=notes,
        )
    ...
    if any(token in text for token in ("wipe", "delete database", "drop table", "destroy evidence", "production database")):
        ...
        return _result(
            ...
            matched_signals=["wipe", "destroy evidence", "production database"],
            notes=notes,
        )
    ...
    if any(token in text for token in ("policy weaken", "silent policy", "remove safety check", "bypass safety", "disable guardrail", "weaken safety")):
        ...
        return _result(
            ...
            matched_signals=["policy weaken", "remove safety check", "disable guardrail"],
            notes=notes,
        )
    ...
    if any(token in text for token in ("early release", "release early", "incomplete control", "urgent release", "ship despite incomplete")):
        ...
        return _result(
            ...
            matched_signals=["early release", "incomplete control", "urgent release"],
            notes=notes,
        )
    ...
    if any(token in text for token in ("refuse", "blocked", "confirm_human", "human review", "maintenance", "internal checklist", "reversible")):
        ...
        return _result(
            ...
            matched_signals=["refuse", "human review", "reversible"],
    ...
    if any(token in text for token in ("safety-critical", "safety_invariant", "invariant", "attest", "self-certify", "self attestation", "claim honesty")):
        ...
        return _result(
            ...
            matched_signals=["attest", "self-certify", "invariant"],
